...
from .get_itsystem_uuid import GetItsystemUuid, GetItsystemUuidItsystems
...
class GraphQLClient(AsyncBaseClient):
    async def get_itsystem_uuid(
        self, itsystem_user_key: str
    ) -> GetItsystemUuidItsystems:
        query = gql(
            """
        ...
        variables: dict[str, object] = {"itsystem_user_key": itsystem_user_key}
        response = await self.execute(query=query, variables=variables)
        data = self.get_data(response)
        return GetItsystemUuid.parse_obj(data).itsystems

    async def get_engagement(
        self, engagement_uuid: UUID, itsystem_uuid: UUID
